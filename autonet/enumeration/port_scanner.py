...
class PortEnumerator(BaseEnumerator):
    def enumerate(
        ...
        for ip_str in scanner.all_hosts():
            host = host_map.get(ip_str)
            if host is None:
                continue
            result = scanner[ip_str]

            hostnames = result.get("hostnames", [])
            if hostnames:
                host.hostname = hostnames[0].get("name", host.hostname)
            host.os_guess = self._extract_os(result)

            for proto in result.all_protocols():
                ports_data = result[proto]
    ...
    def _extract_os(result) -> str | None:
        osmatch_list = result.get("osmatch", [])
        if osmatch_list:
            return osmatch_list[0].get("name")
        osclass_list = result.get("osclass", [])
        if osclass_list:
            return osclass_list[0].get("osfamily")
        return None
